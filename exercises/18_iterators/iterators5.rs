...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize {
    ...
    map.values().fold(0, |acc, val| acc + (*val == value) as usize)
}
...
fn main() {
    ...
    for progress_state in progress_states {
        _ = count_for(&map, progress_state);
    }

    let collection = get_vec_map();
    let progress_states = [Progress::Complete, Progress::Some, Progress::None];
    for progress_state in progress_states {
        _ = count_collection_for(&collection, progress_state);
        _ = count_collection_iterator(&collection, progress_state);
    }
    // It's gettings a lot just to remove the warnings
}
